...
const RELEASE_VERSION = "2.14.121";
...
const state = {
  ...
  runPointDragFrame: null,
  runPointDragPending: null,
  runPointDragStart: null,
  runPointDragMoved: false,
  runPointDragNeedsSelectionRefresh: false,
  runPointDragClickBlockUntil: 0,
  runPointMapClickBlockUntil: 0,
...
const messageFrom = (error, fallback = "Something went wrong. Please try again.") => {
...
const PRESTIGE_LEVEL = XP_LEVEL_CAP + 1;
function rawXpRequiredForLevel(level = 1) {
  const target = Math.max(1, Number(level || 1));
  let total = 0;
  for (let lvl = 2; lvl <= target; lvl += 1) {
    if (lvl <= 20) total += 100 + ((lvl - 2) * 50);
    else if (lvl <= 35) total += 1150 + ((lvl - 21) * 125);
    else total += 3100 + ((lvl - 36) * 250);
  }
  return total;
}
function localXpRequiredForLevel(level = 1) {
  return rawXpRequiredForLevel(Math.min(XP_LEVEL_CAP, Math.max(1, Number(level || 1))));
}
const PRESTIGE_XP_CYCLE = rawXpRequiredForLevel(PRESTIGE_LEVEL);
function localLevelFromXp(xp = 0) {
  const total = Math.max(0, Number(xp || 0));
  let level = 1;
  for (let lvl = 1; lvl <= XP_LEVEL_CAP; lvl += 1) {
    if (localXpRequiredForLevel(lvl) <= total) level = lvl;
  }
  return level;
}
...
function normalizeXpSummary(summary = {}, profile = {}) {
  ...
  const into = Math.max(0, cycle_xp - current);
  return {
    ...summary,
    xp_total: xp,
    prestige_rank,
    cycle_xp,
    prestige_cycle_xp: PRESTIGE_XP_CYCLE,
    level,
...
function levelBadgeHtml(badge = {}, compact = false) {
  ...
  </span>${prestigeRank ? `<span class="prestige-mark ${compact ? "compact" : ""}" title="Prestige ${prestigeRank}"><img src="icons/badges/prestige-01.png?v=2.14.121" alt="Prestige ${prestigeRank}"><b>P${prestigeRank}</b></span>` : ""}</span>`;
}
function levelBadgeImageUrl(level = 1) {
  const safeLevel = Math.min(XP_LEVEL_CAP, Math.max(1, Number(level || 1)));
  if (safeLevel > 45) return "";
  return `icons/badges/level-${String(safeLevel).padStart(2, "0")}.png?v=2.11.77`;
}
...
function levelBadgesGridHtml(summary) {
  const xp = normalizeXpSummary(summary);
  const badges = xp.badges.length ? xp.badges : Array.from({ length: XP_LEVEL_CAP }, (_item, index) => {
    const level = index + 1;
    return { ...localLevelBadge(level), unlocked: level <= xp.level, current: level === xp.level, xp_required: localXpRequiredForLevel(level) };
  });
...
function xpHistoryHtml(rows = []) {
  return rows.length ? `<div class="history-list xp-history-list">${rows.map((row) => `<div class="list-row">
    <div><strong>${escapeHtml(row.item || row.reason || row.source_type)}</strong><small>${escapeHtml(row.reason || row.source_type)} · ${dateLabel(row.event_at)}${row.level_after > row.level_before ? ` · Level ${row.level_after}` : ""}</small></div>
    <span class="points xp-points">${row.xp > 0 ? "+" : ""}${row.xp}<small> XP</small></span>
  </div>`).join("")}</div>` : `<div class="empty compact-empty">No XP history yet.</div>`;
}
...
function showProgressPopup({ kind = "xp", title = "Progress updated", message = "", badge = null } = {}) {
  let stack = document.querySelector(".progress-popup-stack");
  if (!stack) {
    stack = document.createElement("div");
    stack.className = "progress-popup-stack";
    stack.setAttribute("aria-live", "polite");
    document.body.appendChild(stack);
  }
  const popup = document.createElement("div");
  popup.className = `progress-popup ${kind === "badge" ? "badge-popup" : kind === "daily" ? "daily-popup" : kind === "score" ? "score-popup" : "xp-popup"}`;
  const marker = kind === "badge"
    ? `<span class="progress-popup-badge">${badge ? levelBadgeHtml(badge, true) : "★"}</span>`
    : kind === "daily" ? `<span class="progress-popup-xp">✓</span>`
    : kind === "score" ? `<span class="progress-popup-xp">PTS</span>`
  ...
  }, kind === "badge" ? 5000 : kind === "daily" ? 1600 : 3200);
}
...
function showLevelUpModal(summary) {
  ...
  showProgressPopup({
    kind: "badge",
    title: `New badge · Level ${xp.level}`,
    message: xp.level >= XP_LEVEL_CAP ? `You reached the level cap with ${xp.xp_total} ${unit}.` : `${xp.xp_total} ${unit} total · ${xp.xp_needed} to Level ${xp.next_level}.`,
    badge: xp.current_badge,
  });
}
...
async function refreshOwnXpAfterAction({ announceXp = false, trickName = "" } = {}) {
  ...
  try {
    ...
    const xpGained = Math.max(0, Number(summary.xp_total || 0) - beforeXp);
    if (announceXp && xpGained > 0) {
      showProgressPopup({
        kind: "xp",
        title: `+${xpGained} XP`,
...
async function showUpdatedWeeklyScore(pointsChange = 0, trickName = "") {
  if (!Number(pointsChange)) return;
  const leaderboard = await getLeaderboard();
  const row = leaderboard.find((entry) => entry.athlete_id === state.user.id);
  showProgressPopup({
    kind: "score",
    title: `${pointsChange > 0 ? "+" : ""}${pointsChange} leaderboard point${Math.abs(pointsChange) === 1 ? "" : "s"}`,
    message: `${trickName ? `${trickName} · ` : ""}Weekly score ${Number(row?.weekly_points || 0)}`,
  });
}
...
function renderShell() {
  ...
  const sidebarNavHtml = isCoachRole(role)
    ? coachNavGroups.map((group) => `
        <details class="sidebar-nav-group" data-nav-group="${group.id}">
...
async function setupRealtimeSync() {
  ...
  subscriptions.push(
    ...
    { table: "weekly_rider_battles", filter: `challenger_id=eq.${state.user.id}` },
    { table: "weekly_rider_battles", filter: `opponent_id=eq.${state.user.id}` },
  );
...
function isRelevantRealtimePayload(table, payload = {}) {
  ...
  if (table === "weekly_rider_battles") return row.challenger_id === state.user.id || row.opponent_id === state.user.id;
  if (table === "profiles") {
...
async function getLeaderboard() {
  ...
  const request = (async () => {
    ...
    return cacheSet(cacheKey, rows.map((row) => {
      ...
      const xpSummary = riderXpSummary(profile);
      return { ...row, ghost_mode: Boolean(row.ghost_mode ?? profile.ghost_mode), daily_pb_seconds: row.daily_pb_seconds ?? profile.daily_pb_seconds ?? null, xp_total: Number(profile.xp_total || 0), xp_level: xpSummary.level, level: xpSummary.level, level_badge: xpSummary.current_badge };
    }));
...
async function getAthleteHomeLeaderboard() {
  const cacheKey = `leaderboard-home:weekly:${state.user?.id || "anon"}`;
  const cached = cacheGet(cacheKey, 7000);
  if (cached) return cached;
  const existingRequest = state.inFlight.get(cacheKey);
  if (existingRequest) return existingRequest;
  ...
  const request = (async () => {
    const { data, error } = await retryNetworkRequest(
      () => client.rpc("get_weekly_leaderboard"),
      "Home leaderboard load",
      { attempts: 2, timeoutMs: 15000 }
    );
    if (error) {
      console.error("Home leaderboard RPC failed", error);
      return cacheSet(cacheKey, await getLeaderboardFallback(error));
    }
    state.leaderboardFallbackNotified = false;
    return cacheSet(cacheKey, Array.isArray(data) ? data : []);
  })().finally(() => state.inFlight.delete(cacheKey));
...
async function getLeaderboardFallback(cause) {
  ...
  const rows = (profiles || [])
    ...
    level: riderXpSummary(profile).level,
    xp_level: riderXpSummary(profile).level,
    xp_total: Number(profile.xp_total || 0),
    level_badge: riderXpSummary(profile).current_badge,
...
async function getXpHistory(athleteId) {
  const { data, error } = await client.rpc("get_xp_history", { p_athlete_id: athleteId });
  if (error) throw error;
  return data || [];
}
...
async function getTricktionaryPagedRows(queryPage, pageSize = 800) {
  const rows = [];
  ...
  return { data: null, error: new Error("This Tricktionary history is too large to load safely.") };
}
...
async function getTricktionaryData(athleteId) {
  const [profileResult, assignmentsResult, progressResult, attemptsResult, sessionsResult, awardsResult, percentageAttemptsResult, landingHistoryResult, landedAttemptsResult] = await Promise.all([
    ...
    getTricktionaryPagedRows((from, to) => client.from("assignment_attempts").select("*").eq("athlete_id", athleteId).order("attempted_at", { ascending: false }).order("id", { ascending: true }).range(from, to)),
...
async function getPublicRiderBattleRecord(athleteId) {
  const { data, error } = await client.rpc("get_public_rider_battle_record", { p_athlete_id: athleteId });
  if (error) throw error;
...
async function getWeeklyAssignments(athleteId, { includeAssignmentAttempts = true } = {}) {
  ...
  const request = (async () => {
    if (state.user?.id) {
      ...
      if (rolloverError && !/not allowed/i.test(rolloverError.message || "")) throw rolloverError;
    }
    const assignmentAttemptsRequest = includeAssignmentAttempts
    ...
    const [{ data, error }, { data: progress, error: progressError }, { data: awards, error: awardsError }, { data: percentageAttempts, error: percentageError }, { data: assignmentAttempts, error: attemptError }] = await Promise.all([
      ...
      client.from("assignment_progress").select("assignment_id,athlete_id,progress_date,streak_count,completed_at,updated_at").eq("athlete_id", athleteId),
      client.from("assignment_point_awards").select("id,athlete_id,session_id,assignment_id,award_key,points,created_at,venue").eq("athlete_id", athleteId).gte("created_at", new Date(Date.now() - (8 * 24 * 60 * 60 * 1000)).toISOString()),
      client.from("percentage_attempts").select("id,assignment_id,athlete_id,attempt_number,landed,created_at").eq("athlete_id", athleteId).order("attempt_number", { ascending: true }),
      assignmentAttemptsRequest,
    ]);
    if (error) throw error;
...
async function getTrickRequestsForAthlete(athleteId, statuses = []) {
  let query = client.from("trick_requests").select("id,athlete_id,category,venue,trick_name,notes,status,created_at,reviewed_at").eq("athlete_id", athleteId).order("created_at", { ascending: false });
...
async function getEventCoursePhoto(eventId = "") {
  ...
  const key = `event-course:${userId}:${eventId}`;
...
const RUN_SUMMARY_SELECT = "id,athlete_id,coach_id,created_by,title,venue,plan_type,contest_item_id,created_at,updated_at,archived_at,run_status";

async function getRiderRunSummaries(athleteId) {
  const { data, error } = await client.from("run_plans").select(RUN_SUMMARY_SELECT)
    .eq("athlete_id", athleteId).order("updated_at", { ascending: false }).limit(100);
  if (error) throw error;
  return data || [];
}

async function getEventCoachRoster() {
  ...
  const { data, error: profileError } = await client.from("profiles").select("id,display_name,country_code")
...
async function getActiveCoachGroupSession() {
  const { data, error } = await client.from("coach_group_sessions")
    .select("id,coach_id,group_name,venue,status,started_at,paused_at,total_paused_seconds,ended_at,coach_group_session_participants(group_session_id,athlete_id,training_session_id,joined_at,last_activity_at,daily_finished_at,daily_finish_seconds)")
    .eq("coach_id", state.user.id)
...
const categoryRewardLabels = {
  daily: "Full list · 1 pt",
  one_bang: "2 pts each",
  dialled: "2 pts each",
  lines: "3 pts each",
  percentage: "Up to 3 pts",
  foam_pit: "Practice only",
  bonus: "5 pts each",
};
...
const plannerStatusLabels = {
  draft: "Legacy draft",
  draft_next_week: "Draft",
  scheduled_next_week: "Scheduled",
  published: "Scheduled",
};
const plannerStatusRank = {
  scheduled_next_week: 0,
  published: 1,
  draft_next_week: 2,
  draft: 3,
};
...
function profileGroupNames(profile = {}) {
  ...
  add(source.groupNames);
  add(source.group_names);
  add(source.groupName);
  add(source.group_name);
...
const heatStatuses = {
  ...
  away: { label: "On holiday / away", dot: "grey", icon: "●" },
};
...
function parkKingCardHtml(king, venue = "", { id = "", compact = false, loading = false, requestId = "" } = {}) {
  ...
  const content = loading
    ...
    : king
    ? `<div class="park-king-rider"><span class="park-king-crown" aria-hidden="true">♛</span>${avatarHtml(king, "park-king-avatar")}<div><span class="park-champion-tag">Current champion</span><strong>${escapeHtml(king.display_name)}</strong><small>${Number(king.points || 0)} park points</small></div></div>`
  ...
  return ordered.map((assignment) => {
    ...
    const controls = interactive ? `
      <div class="assignment-actions">
        <button class="assignment-check ${complete ? "complete" : ""}" type="button" aria-label="${label}" title="${label}" data-assignment-action="${action}" data-assignment-id="${assignment.id}" data-assignment-category="${assignment.category}">${complete ? "✓" : ""}</button>
...
async function acceptTrickRequest(event) {
  ...
  try {
    const { error } = await client.rpc("accept_trick_request_into_plan", { p_request_id: requestId });
    if (error) throw error;
...
function weekSummaryHtml(assignments, awards) {
  ...
  const dailyDays = Math.min(7, dailyCompletionCount(awards));
  return `<section class="panel simple-summary">
    ...
    <div class="weekly-daily-days"><span>Daily Tricks</span><strong>${dailyDays}/7 <small>days</small></strong></div>
    <p>Weekly program completed ${completedWeekly}/${weeklyItems.length || 0}. Daily Tricks show days completed across the week—riders are not expected to train all seven days.</p>
  </section>`;
}
...
const TRICKTIONARY_STANDARD_SUBCATEGORIES = [
...
function tricktionaryMeta(profile = {}) {
  ...
  manualTricktionary(profile).forEach((trick) => {
    const mergedFrom = Array.isArray(trick?.mergedFrom) ? trick.mergedFrom : [];
    const title = String(trick?.title || trick?.name || "").trim();
    if (trick?.source !== "merged" || mergedFrom.length < 2 || !title) return;
    const canonicalKey = normalizeTrickKey(title);
    if (!canonicalKey) return;
    mergedFrom.forEach((sourceTitle) => {
      const sourceKey = normalizeTrickKey(sourceTitle);
      if (sourceKey && sourceKey !== canonicalKey) legacyAliases[sourceKey] = canonicalKey;
    });
    legacyTitles[canonicalKey] = title;
    legacyCategories[canonicalKey] = safeTricktionaryCategory(trick.tricktionaryCategory || trick.categoryOverride || "new");
  });
  return {
    categories: {
      ...legacyCategories,
      ...(meta.categories && typeof meta.categories === "object" ? meta.categories : {}),
    },
    aliases: {
      ...legacyAliases,
      ...(meta.aliases && typeof meta.aliases === "object" ? meta.aliases : {}),
    },
    titles: {
      ...legacyTitles,
...
function resolveTricktionaryAlias(key = "", aliases = {}) {
  let current = normalizeTrickKey(key);
  const path = [];
  const positions = new Map();
  while (current && aliases?.[current] && path.length < 50) {
    ...
    path.push(current);
    const next = normalizeTrickKey(aliases[current]);
...
function tricktionarySubcategory(entry = {}, category = "new") {
  ...
  if (category === "air") {
    ...
    if (/(^|\D)(360|540|900)(?!\d)/.test(title)) return "spins";
    return "other";
  }
...
function landedTricktionaryEntries(data = {}) {
  ...
  manualTricktionary(data.profile).forEach((trick) => {
    const title = String(trick.title || trick.name || "").trim();
    if (!title) return;
    if (trick.source === "merged" && Array.isArray(trick.mergedFrom) && trick.mergedFrom.length >= 2) return;
...
function tricktionaryCardHtml(entry = {}, attemptMap = new Map(), editable = false) {
  const key = entry.key || normalizeTrickKey(entry.title);
...
function tricktionaryBoardHtml(entries = [], attempts = [], options = {}) {
  ...
  const rowsHtml = (rows) => rows.length
    ...
    : `<div class="empty compact-empty">Nothing in this group yet.</div>`;
  const zoneHtml = (section, isTop = false, index = 0) => {
    const rows = grouped[section.id] || [];
    if (isTop) return `<section class="tricktionary-zone new-zone" data-tricktionary-drop-category="${escapeHtml(section.id)}">
      <div class="tricktionary-zone-header">
        <div class="tricktionary-zone-heading"><b>00</b><div><h3>${escapeHtml(section.label)}</h3><small>${escapeHtml(section.hint)}</small></div></div>
        <span class="tricktionary-zone-count">${rows.length}</span>
      </div>
    ...
    return `<details class="tricktionary-zone tricktionary-category-accordion" data-tricktionary-drop-category="${escapeHtml(section.id)}">
      <summary class="tricktionary-zone-header">
        <div class="tricktionary-zone-heading"><b>${String(index).padStart(2, "0")}</b><div><h3>${escapeHtml(section.label)}</h3><small>${escapeHtml(section.hint)}</small></div></div>
        <span class="tricktionary-zone-count">${rows.length}</span>
      </summary>
      <div class="tricktionary-zone-body">${subcategoryHtml}</div>
    </details>`;
  };
...
async function mergeTricktionaryEntries(athleteId, sourceEntry = {}, targetEntry = {}, category, subcategory = "") {
  ...
  const combinedTotal = Math.max(1, Number(sourceEntry.landedCount || 1)) + Math.max(1, Number(targetEntry.landedCount || 1));
  ...
  if (error) throw error;
  cacheClear("roster");
  notify(`Tricks merged — ${combinedTotal} landed results are now combined under ${cleanTitle.slice(0, 120)}.`);
  return true;
}

function tricktionaryDragEntry(card) {
  if (!card) return null;
  let memberKeys = [];
  try {
    memberKeys = JSON.parse(decodeURIComponent(card.dataset.trickMemberKeys || "%5B%5D"));
  } catch (_) {}
  return {
...
function bindTricktionaryBoard({ athleteId, refresh }) {
  ...
  board.querySelectorAll("[data-tricktionary-card]").forEach((card) => {
    card.addEventListener("dragstart", (event) => {
      const payload = tricktionaryDragEntry(card);
    ...
    card.addEventListener("pointerdown", (event) => {
      ...
      pointerPayload = { ...tricktionaryDragEntry(card), active: false, startX: event.clientX, startY: event.clientY };
    ...
    card.addEventListener("pointermove", (event) => {
      if (!pointerPayload) return;
      if (!pointerPayload.active) {
        if (Math.hypot(event.clientX - pointerPayload.startX, event.clientY - pointerPayload.startY) > 10) {
  ...
  board.querySelectorAll("[data-tricktionary-drop-category], [data-tricktionary-drop-subcategory]").forEach((zone) => {
    ...
    zone.addEventListener("dragleave", (event) => {
      ...
      zone.querySelectorAll(".merge-target").forEach((node) => node.classList.remove("merge-target"));
    });
...
async function renderTricktionary() {
  ...
  document.querySelector("#view").innerHTML = `
    ...
    <section class="panel">
      ...
      ${tricktionaryBoardHtml(entries, data.attempts, { editable: true, profile: data.profile })}
    </section>
    <section class="panel">
...
async function getWeeklyRiderBattles() {
  const { data, error } = await client.rpc("get_my_rider_battles");
  if (error) throw error;
  return Array.isArray(data) ? data : [];
}

async function getRiderBattleHistory() {
  return (await getWeeklyRiderBattles()).filter((battle) => battle.status === "completed");
}

function riderHeadToHeadRecord(history = [], opponentId = "") {
  const meetings = history.map((battle) => ({
    mine: (battle.participants || []).find((rider) => rider.athlete_id === state.user.id),
    opponent: (battle.participants || []).find((rider) => rider.athlete_id === opponentId),
  })).filter(({ mine, opponent }) => mine && opponent && mine.team_number !== opponent.team_number);
  return {
    wins: meetings.filter(({ mine, opponent }) => mine.is_winner === true && opponent.is_winner === false).length,
    losses: meetings.filter(({ mine, opponent }) => mine.is_winner === false && opponent.is_winner === true).length,
  };
}

function riderBattleRecord(history = []) {
  const mine = history.map((battle) => (battle.participants || []).find((participant) => participant.athlete_id === state.user.id)).filter(Boolean);
  const wins = mine.filter((participant) => participant.is_winner === true).length;
  const losses = mine.filter((participant) => participant.is_winner === false).length;
  const decided = wins + losses;
  return { wins, losses, winPercent: decided ? Math.round((wins / decided) * 100) : 0 };
}
...
function battleDashboardAlertsHtml(battles = []) {
  ...
  return `<section class="battle-dashboard-alerts" aria-label="Battle requests">${incoming.map((battle) => { const challengers = (battle.participants || []).filter((participant) => participant.team_number === 1).map(battleParticipantFirstName).join(" + "); return `<article class="battle-dashboard-alert">
    ...
    <div><div class="eyebrow">New ${battleFormatLabel(battle)} request</div><strong>${escapeHtml(challengers || "Your crew")} challenged you</strong><small>Tap to accept ✓ or decline ×.</small></div>
...
function battleTeamNumbers(battle = {}) {
  const count = Math.max(2, Math.min(3, Number(battle.team_count) || Math.max(2, ...(battle.participants || []).map((rider) => Number(rider.team_number) || 1))));
...
function weeklyBattleCardHtml(battle, _pointsByRider = new Map(), battleHistory = []) {
  ...
  const statusCopy = battle.status === "pending"
    ...
    : battle.status === "accepted" ? `Live · ${durationCopy} · ends ${dateLabel(battle.ends_at)}`
      : battle.status === "completed" ? (mine?.is_winner === true ? `Your team won · ${mine.points_delta > 0 ? `+${mine.points_delta}` : "points added"}` : mine?.is_winner === false ? "Another team won" : "Battle finished level")
        : "Challenge declined";
...
async function getMyWeeklyChallenge() {
  const { data, error } = await client.rpc("get_my_weekly_challenge");
  if (error) throw error;
  return data || null;
}
...
async function renderChallenges() {
  ...
  const battleLimitReached = activeBattleCount >= 3;
  const availableRiders = leaderboard.filter((row) => row.athlete_id !== state.user.id && !row.ghost_mode);
...
async function respondWeeklyRiderBattle(event) {
  const button = event.currentTarget;
  const restoreButton = setButtonBusy(button, button.dataset.battleResponse === "accepted" ? "Accepting..." : "Declining...");
...
async function forfeitWeeklyRiderBattle(event) {
  ...
  const { data, error } = await client.rpc("forfeit_rider_battle", { p_battle_id: button.dataset.forfeitBattle });
  restoreButton();
  if (error) return notify(messageFrom(error), "error");
  notify(data === "accepted" ? "Your team forfeited. The remaining sides are still competing." : "Battle forfeited. The result and leaderboard have been updated.");
  await renderChallenges();
}
...
async function removeManualTrick(event) {
  ...
  const { error } = await client.rpc("remove_manual_tricktionary_entry", { p_athlete_id: state.user.id, p_entry_id: id });
  if (error) return notify(messageFrom(error), "error");
  cacheClear("roster");
...
async function renderParentTricktionary() {
  ...
  document.querySelector("#view").innerHTML = `
    ...
    <section class="panel parent-child-card">
      ...
      ${tricktionaryBoardHtml(entries, data.attempts, { profile: data.profile })}
      <div class="settings-divider"></div>
...
async function openRiderSavedRuns(event) {
  ...
  const restore = setButtonBusy(button, "Loading saved runs…");
  const userId = state.user.id;
  try {
    ...
    closeContestEventModal();
    const backdrop = document.createElement("div");
    backdrop.id = "contest-event-backdrop"; backdrop.className = "contest-event-backdrop";
    backdrop.innerHTML = `<section class="contest-event-modal" role="dialog" aria-modal="true" aria-labelledby="saved-runs-title"><header class="contest-event-modal-head"><div><h2 id="saved-runs-title">Your saved runs</h2><p>Private to you and your linked coach</p></div><button type="button" class="contest-event-modal-close" data-close-contest-event aria-label="Close saved runs">×</button></header>${riderSavedRunsHtml(data || [])}</section>`;
    document.body.append(backdrop); document.documentElement.classList.add("contest-event-open");
    backdrop.querySelector("[data-close-contest-event]").onclick = closeContestEventModal;
...
async function openProgressRun(button) {
  ...
  try {
    let query = client.from("run_plans").select("*").eq("id", button.dataset.openProgressRun);
    if (!button.dataset.openArchivedRun) query = query.is("archived_at", null);
    const { data, error } = await withTimeout(query.single(), "Open saved run", 15000);
    if (error) throw error;
    openCoachEventRunModal([data], button.dataset.riderName || state.profile?.display_name || "Rider", { title: data.title, includeArchived: Boolean(button.dataset.openArchivedRun) });
  } catch (error) { notify(messageFrom(error), "error"); }
  finally { restore(); }
}
...
function scoreRankingCard(weeklyPoints, rank, riderCount, battleRecord = null, athleteId = state.user?.id) {
  return `<article class="stat-card score-ranking-card ${battleRecord ? "with-battle-record" : ""}">
    ...
    <div class="score-ranking-stat">
      ...
      <div class="stat-value">${rank ? `#${escapeHtml(rank)}` : "-"}</div>
      <div class="stat-foot">${escapeHtml(Number(riderCount || 0))} riders on board</div>
    </div>
    ${battleRecord ? `<div class="score-ranking-stat battle-ranking-stat">
      <div class="stat-label">Battle record</div>
      <div class="stat-value">${escapeHtml(Number(battleRecord.wins || 0))}<small>W</small> — ${escapeHtml(Number(battleRecord.losses || 0))}<small>L</small></div>
      <div class="stat-foot">${escapeHtml(Number(battleRecord.winPercent || 0))}% win rate</div>
    </div>` : ""}
  </article>`;
}
...
async function recordAssignmentAction(event) {
  ...
  if (error) {
    ...
    button.classList.toggle("complete", Boolean(wasComplete));
    button.textContent = wasComplete ? "✓" : "";
    return notify(messageFrom(error), "error");
  }
  ...
  document.querySelector("#view").innerHTML = `
    ...
    <section class="panel board-rankings-panel">
      <div class="panel-head board-rankings-head">
        <div><div class="panel-title">${boardTitle}</div><div class="panel-meta">${boardMeta} · Badges and levels follow original lifetime XP.</div></div>
...
async function renderPublicAthleteProfile() {
  ...
  const [profile, battleRecord, leaderboard] = await Promise.all([
    getPublicAthleteProfile(state.publicAthleteId),
    getPublicRiderBattleRecord(state.publicAthleteId).catch(() => ({ wins: 0, losses: 0, win_percent: 0 })),
    getLeaderboard(),
  ]);
  if (!profile) {
...
function contestEventCardsHtml(events = [], runs = [], attendance = [], roster = [], viewOptions = {}) {
  ...
  return `<div class="contest-event-grid">${events.map((item) => {
    ...
    return `<article class="contest-event-card ${going ? "is-going" : ""} ${coachView ? "coach-merge-event" : ""} ${mergeSelected ? "merge-source-selected" : ""}" data-contest-event-card data-contest-event-id="${escapeHtml(item.id)}" data-event-search="${escapeHtml(searchText)}" ${coachView ? 'draggable="true"' : ""}>
      ...
      ${athleteView ? `<div class="contest-event-actions"><button class="secondary-btn compact-btn" type="button" data-rider-saved-runs="${escapeHtml(item.id)}">VIEW MY SAVED RUNS${linkedRuns ? ` (${linkedRuns})` : ""}</button><button class="${going ? "secondary-btn is-going" : "primary-btn"} compact-btn" type="button" data-toggle-contest-attendance="${escapeHtml(item.id)}" data-attending="${going}">${going ? "✓ GOING" : "+ I'M GOING"}</button><button class="secondary-btn compact-btn contest-build-button" type="button" data-build-event-run="${escapeHtml(item.id)}" ${contestEventDataAttributes(item)}>${linkedRuns ? "ADD PRIVATE RUN" : "BUILD PRIVATE RUN"}</button></div>` : ""}
...
function coachEventRunViewerHtml(runs = [], athleteName = "Rider", item = {}) {
  const savedRuns = runs.filter((run) => !run.archived_at || item.includeArchived);
...
async function renderContests() {
  ...
  const [{ events, attendance }, roster, riderRuns] = await Promise.all([
    getSharedUpcomingEventData(),
    coachView ? getEventCoachRoster() : Promise.resolve([]),
    state.profile?.role === "athlete" ? getRiderRunSummaries(state.user.id) : Promise.resolve([]),
  ]);
  if (renderVersion !== state.contestsRenderVersion || state.view !== "contests") return;
...
function coachBattleCardHtml(battle) {
  ...
  const timing = battle.status === "accepted" && battle.ends_at
    ...
    : battle.status === "pending" ? `Requested ${dateLabel(battle.created_at)}`
      : `${duration} day battle`;
  const teams = battleTeamNumbers(battle);
  const teamNames = teams.map((team) => participants.filter((rider) => rider.team_number === team).map((rider) => escapeHtml(rider.display_name)).join(" + ") || `Team ${team}`);
  const teamScores = teams.map((team) => battleTeamScore(battle, team));
  ...
  return `<details class="coach-battle-view-card battle-hq-card ${escapeHtml(battle.status)}" data-battle-id="${escapeHtml(battle.id)}" data-battle-hq-status="${escapeHtml(archived ? "archived" : battle.status)}" data-battle-hq-riders="${escapeHtml(searchableRiders)}">
    <summary class="coach-battle-card-summary">
      ...
      <span class="coach-battle-summary-format">${battleFormatLabel(battle)} · ${battlePrizePoints(battle)} pts to win</span>
...
function coachArchivedBattleSection(battles = []) {
  return `<details id="coach-battle-archive" class="panel coach-battle-view-section coach-battle-archive tone-blue"><summary><span><strong>Archived battles</strong><small>Finished results kept safely out of the main view</small></span><span class="pill">${battles.length}</span></summary><div class="coach-battle-view-list">${battles.length ? battles.map(coachBattleCardHtml).join("") : `<div class="empty compact-empty">No archived battles yet.</div>`}</div></details>`;
}

// Coach overview reads recorded challenge results without triggering rider awards.
async function getCoachWeeklyChallenges(roster) {
  try {
    const now = new Date();
    const [{ data, error }, riders] = await Promise.all([
      client.from("weekly_challenges")
        .select("id,title,description,category,completion_rule,target_count,reward_points,audience_group,starts_at,ends_at,status")
        .in("status", ["active", "scheduled"]).gte("ends_at", now.toISOString()).order("starts_at", { ascending: true }),
      Promise.resolve(roster),
    ]);
    if (error) throw error;
    const rows = Array.isArray(data) ? data : [];
    const active = rows.find(row => row.status === "active" && new Date(row.starts_at) <= now && new Date(row.ends_at) >= now) || null;
    const upcoming = rows.filter(row => row.status === "scheduled" && new Date(row.starts_at) > now && new Date(row.ends_at) >= now);
    const uniqueRiders = [...new Map((riders || []).map(rider => [rider.id, rider])).values()];
    const result = { active, upcoming, completedRiders: [], eligibleCount: uniqueRiders.length, completionError: false, error: false };
    if (active && uniqueRiders.length) {
      try {
        let audienceRiders = uniqueRiders;
        if (active.audience_group) {
          // Roster display groups may be defaults. Count actual memberships in this coach's group.
          const { data: memberships, error: membershipError } = await client.from("coach_athlete_groups")
            .select("athlete_id").eq("coach_id", state.user.id).eq("group_name", active.audience_group).in("athlete_id", uniqueRiders.map(rider => rider.id));
          if (membershipError) throw membershipError;
          const memberIds = new Set((memberships || []).map(row => row.athlete_id));
          audienceRiders = uniqueRiders.filter(rider => memberIds.has(rider.id));
        }
        result.eligibleCount = audienceRiders.length;
        if (audienceRiders.length) {
          const { data: completions, error: completionError } = await client.from("weekly_challenge_completions")
            .select("challenge_id,athlete_id,awarded_at").eq("challenge_id", active.id).in("athlete_id", audienceRiders.map(rider => rider.id));
          if (completionError) throw completionError;
          const completedIds = new Set((completions || []).filter(row => row.challenge_id === active.id).map(row => row.athlete_id));
          result.completedRiders = audienceRiders.filter(rider => completedIds.has(rider.id));
        }
      } catch { result.completionError = true; }
    }
    return result;
  } catch {
    return { active: null, upcoming: [], completedRiders: [], eligibleCount: 0, completionError: false, error: true };
  }
}

function coachWeeklyChallengeCardHtml(challenge, summary = null) {
  const audience = challenge.audience_group
    ? coachGroups.find(([id]) => id === challenge.audience_group)?.[1] || challenge.audience_group
    : "All riders";
  const target = challenge.completion_rule === "percentage_perfect"
    ? `${Number(challenge.target_count)} percentage tricks at 10/10`
    : `${Number(challenge.target_count)} × ${categoryDisplayInfo(challenge.category).label}`;
  const completed = summary?.completedRiders || [];
  return `<article class="coach-weekly-card">
    <div class="coach-weekly-card-head"><div><div class="eyebrow">${summary ? "Live this week" : "Scheduled"}</div><h3>${escapeHtml(challenge.title)}</h3></div><span class="coach-challenge-reward">+${Number(challenge.reward_points)} pts</span></div>
    <p>${escapeHtml(challenge.description || "")}</p>
    <dl class="coach-challenge-meta"><div><dt>Target</dt><dd>${escapeHtml(target)}</dd></div><div><dt>Audience</dt><dd>${escapeHtml(audience)}</dd></div><div><dt>${summary ? "Ends" : "Starts"}</dt><dd>${escapeHtml(dateLabel(summary ? challenge.ends_at : challenge.starts_at))}</dd></div></dl>
    ${summary ? summary.completionError
      ? `<p class="coach-challenge-error" role="status">Completion results couldn't load. <button type="button" class="secondary-btn compact-btn" data-retry-coach-challenges>Retry</button></p>`
      : summary.eligibleCount ? `<div class="coach-challenge-progress"><label for="coach-weekly-progress">${challenge.audience_group ? `${completed.length} of ${summary.eligibleCount} riders in your ${escapeHtml(audience)} completed` : `${completed.length} of your ${summary.eligibleCount} riders completed`}</label><progress id="coach-weekly-progress" value="${completed.length}" max="${summary.eligibleCount}"></progress></div>
        ${completed.length ? `<details id="coach-challenge-riders" class="coach-challenge-riders"><summary>Completed riders · ${completed.length}<span class="accordion-caret" aria-hidden="true">⌄</span></summary><ul>${completed.map(rider => `<li><strong>${escapeHtml(rider.display_name)}</strong><span>Reward awarded</span></li>`).join("")}</ul></details>` : ""}`
      : `<p class="coach-challenge-empty">${challenge.audience_group ? `No riders are assigned to your ${escapeHtml(audience)} group.` : "There are no riders in your crew yet."}</p>` : ""}
  </article>`;
}

function coachWeeklyChallengeHtml(summary) {
  if (summary.error) return `<div class="coach-challenge-error" role="status"><strong>Weekly challenges couldn't load</strong><p>Your battles are still available below.</p><button class="secondary-btn compact-btn" type="button" data-retry-coach-challenges>Retry challenges</button></div>`;
  return `${summary.active ? coachWeeklyChallengeCardHtml(summary.active, summary) : `<div class="coach-challenge-empty"><strong>No active weekly challenge</strong><p>Set a training target for your riders with New challenge.</p></div>`}
    ${summary.upcoming.length ? `<details id="coach-challenge-upcoming" class="coach-challenge-upcoming"><summary><strong>Up next · ${summary.upcoming.length}</strong><span class="accordion-caret" aria-hidden="true">⌄</span></summary>${summary.upcoming.map(challenge => coachWeeklyChallengeCardHtml(challenge)).join("")}</details>` : ""}`;
}

function captureCoachBattleViewState(view) {
  if (!view.querySelector(".coach-challenges-page")) return null;
  return {
    openDetails: [...view.querySelectorAll("details[open]")].map(el => el.id || el.dataset.battleId).filter(Boolean),
    search: view.querySelector("#battle-hq-search")?.value || "",
    historySearch: view.querySelector("#battle-history-search")?.value || "",
    filter: view.querySelector("[data-battle-hq-filter].active")?.dataset.battleHqFilter || "all",
    scrollY: window.scrollY,
  };
}

function restoreCoachBattleViewState(view, saved) {
  if (!saved) return;
  for (const [selector, value] of [["#battle-hq-search", saved.search], ["#battle-history-search", saved.historySearch]]) {
    const input = view.querySelector(selector);
    if (input) { input.value = value; input.dispatchEvent(new Event("input")); }
  }
  [...view.querySelectorAll("[data-battle-hq-filter]")].find(el => el.dataset.battleHqFilter === saved.filter)?.click();
  view.querySelectorAll("details").forEach(el => { el.open = saved.openDetails.includes(el.id || el.dataset.battleId); });
...
async function refreshCoachBattleScores() {
  ...
  try { await renderCoachBattleViewer(); }
...
async function renderCoachBattleViewer() {
  ...
  const archived = rows.filter((battle) => battle.archived_at);
  const live = current.filter((battle) => battle.status === "accepted");
  const pending = current.filter((battle) => battle.status === "pending");
  const finished = current.filter((battle) => ["completed", "declined"].includes(battle.status));
  ...
  const view = document.querySelector("#view");
  // Read expansion choices after loading, so clicks made while refreshing survive.
  const savedView = captureCoachBattleViewState(view);
  view.innerHTML = `<div class="coach-challenges-page">
    <header class="coach-challenges-header"><div><div class="eyebrow">Coach overview</div><h1>Battles &amp; Challenges</h1><p>Weekly training targets, live matchups and past results.</p></div><button class="secondary-btn battle-hq-refresh" type="button" id="refresh-coach-battles" aria-label="Refresh battles and challenges">↻</button></header>
    <section class="coach-weekly-panel" aria-labelledby="coach-weekly-heading">
      <div class="coach-section-heading"><div><h2 id="coach-weekly-heading">Weekly challenges</h2><p>A shared target. A little extra motivation.</p></div><button class="secondary-btn" type="button" id="coach-create-weekly-challenge">＋ New challenge</button></div>
      ${coachWeeklyChallengeHtml(challengeSummary)}
    </section>
    <section class="coach-current-battles" aria-labelledby="coach-battles-heading">
      <div class="coach-section-heading"><div><h2 id="coach-battles-heading">Rider battles</h2><p>${live.length} live · ${pending.length} waiting · ${riderCount} riders competing</p></div><button class="primary-btn" type="button" id="coach-create-battle">＋ Create battle</button></div>
      <div class="battle-hq-toolbar"><div class="battle-hq-filters" role="group" aria-label="Battle status"><button class="active" type="button" data-battle-hq-filter="all" aria-pressed="true">Current <b>${live.length + pending.length}</b></button><button type="button" data-battle-hq-filter="accepted" aria-pressed="false">Live <b>${live.length}</b></button><button type="button" data-battle-hq-filter="pending" aria-pressed="false">Waiting <b>${pending.length}</b></button></div><label class="battle-hq-search"><span class="sr-only">Search current battles by rider</span><input id="battle-hq-search" type="search" placeholder="Find rider in a battle…"></label></div>
      <div class="battle-hq-results">
        ${coachBattleSection("Live battles", "Tap a matchup for scores and rider contributions", live)}
        ${coachBattleSection("Pending requests", "Waiting for riders to accept", pending)}
      </div>
      <div id="coach-current-battles-empty" class="empty compact-empty" role="status"></div>
    </section>
    <details id="coach-battle-history" class="coach-battle-history">
      <summary class="coach-history-heading"><span><strong>Finished battle history</strong><small>Past results and archived battles</small></span><span class="pill">${finished.length + archived.length}</span><span class="accordion-caret" aria-hidden="true">⌄</span></summary>
      <div class="coach-history-body">
        <label class="battle-hq-search"><span>Find a past battle</span><input id="battle-history-search" type="search" placeholder="Search history by rider…"></label>
        <h3>Finished · ${finished.length}</h3>
        <div class="coach-battle-view-list" data-finished-battle-list>${finished.map(coachBattleCardHtml).join("")}</div>
        <p id="coach-finished-battles-empty" class="empty compact-empty" ${finished.length ? "hidden" : ""}>No finished battles yet.</p>
        ${coachArchivedBattleSection(archived)}
        <p id="coach-history-search-empty" class="empty compact-empty" role="status" hidden>No past battles match this rider.</p>
      </div>
    </details>
  </div>`;
  ...
  const applyBattleFilters = () => {
    const status = view.querySelector("[data-battle-hq-filter].active")?.dataset.battleHqFilter || "all";
    const query = String(view.querySelector("#battle-hq-search")?.value || "").trim().toLowerCase();
    const cards = [...view.querySelectorAll(".battle-hq-results .battle-hq-card")];
    cards.forEach((card) => {
      const statusMatch = status === "all" || card.dataset.battleHqStatus === status;
      card.hidden = !statusMatch || Boolean(query && !String(card.dataset.battleHqRiders || "").includes(query));
    });
    view.querySelectorAll("[data-battle-hq-section]").forEach((section) => {
    ...
    const empty = view.querySelector("#coach-current-battles-empty");
    empty.hidden = cards.some(card => !card.hidden);
    empty.textContent = query ? "No current battles match this rider." : status === "accepted" ? "No live battles right now." : status === "pending" ? "No requests waiting for approval." : "No current battles. Create a matchup when your riders are ready.";
  };
  view.querySelectorAll("[data-battle-hq-filter]").forEach((button) => button.addEventListener("click", () => {
    view.querySelectorAll("[data-battle-hq-filter]").forEach((item) => { item.classList.toggle("active", item === button); item.setAttribute("aria-pressed", String(item === button)); });
    applyBattleFilters();
  }));
  view.querySelector("#battle-hq-search")?.addEventListener("input", applyBattleFilters);
  view.querySelector("#battle-history-search")?.addEventListener("input", () => {
    const query = String(view.querySelector("#battle-history-search").value || "").trim().toLowerCase();
    const cards = [...view.querySelectorAll("#coach-battle-history .battle-hq-card")];
    cards.forEach(card => { card.hidden = Boolean(query && !String(card.dataset.battleHqRiders || "").includes(query)); });
    const archive = view.querySelector("#coach-battle-archive");
    const archivedMatches = [...archive.querySelectorAll(".battle-hq-card")].filter(card => !card.hidden).length;
    archive.hidden = Boolean(query && !archivedMatches);
    const archiveCount = archive.querySelector(".pill");
    if (archiveCount) archiveCount.textContent = archivedMatches;
    const finishedMatches = [...view.querySelectorAll("[data-finished-battle-list] .battle-hq-card")].filter(card => !card.hidden).length;
    const finishedEmpty = view.querySelector("#coach-finished-battles-empty");
    finishedEmpty.hidden = Boolean(finishedMatches) || Boolean(query);
    view.querySelector("#coach-history-search-empty").hidden = !query || cards.some(card => !card.hidden);
  });
  applyBattleFilters();
...
function refreshCoachBattleSurface() {
  return state.view === "sessionViewer" ? renderSessionViewer() : renderCoachBattleViewer();
}

function bindCoachBattleControls() {
  document.querySelectorAll("[data-coach-accept-battle]").forEach((button) => button.addEventListener("click", respondCoachRiderBattle));
  document.querySelectorAll("[data-archive-coach-battle]").forEach((button) => button.addEventListener("click", setCoachBattleArchived));
  document.querySelectorAll("[data-delete-coach-battle]").forEach((button) => button.addEventListener("click", deleteCoachBattle));
}

async function respondCoachRiderBattle(event) {
  const button = event.currentTarget;
  const riderName = button.textContent.replace(/^✓\s*Accept for\s*/i, "").trim() || "rider";
  const restoreButton = setButtonBusy(button, "Accepting...");
  const { data, error } = await client.rpc("coach_respond_rider_battle", { p_battle_id: button.dataset.coachAcceptBattle, p_athlete_id: button.dataset.coachAcceptAthlete, p_response: "accepted" });
  restoreButton();
  if (error) return notify(messageFrom(error), "error");
  notify(data === "accepted" ? `${riderName} accepted. Everyone is ready and the battle is now live.` : `${riderName} accepted. Waiting for the remaining riders.`);
  await refreshCoachBattleSurface();
}

async function setCoachBattleArchived(event) {
  const button = event.currentTarget;
  const archived = button.dataset.battleArchived === "true";
  if (!archived && !window.confirm("Archive this finished battle? Its result, points and win/loss record will stay saved.")) return;
  const restoreButton = setButtonBusy(button, archived ? "Restoring..." : "Archiving...");
  const { error } = await client.rpc("set_rider_battle_archived", { p_battle_id: button.dataset.archiveCoachBattle, p_archived: !archived });
  restoreButton();
  if (error) return notify(messageFrom(error), "error");
  notify(archived ? "Battle restored to finished results." : "Battle archived. Its result and points are still saved.");
  await refreshCoachBattleSurface();
}
...
function showWeeklyChallengeBuilder(refresh = renderCoachBattleViewer) {
  document.querySelector("#weekly-challenge-builder-modal")?.remove();
  const backdrop = document.createElement("div");
  backdrop.id = "weekly-challenge-builder-modal";
  backdrop.className = "battle-intro-backdrop";
  backdrop.innerHTML = `<section class="battle-intro-card coach-battle-builder-card" role="dialog" aria-modal="true" aria-label="Create weekly challenge"><div class="eyebrow">Weekly crew challenge</div><h2>Set the next target</h2><p>Riders earn 5 leaderboard points automatically when they complete it.</p><form id="weekly-challenge-builder-form"><div class="field"><label>Challenge name</label><input name="title" maxlength="80" required placeholder="e.g. Line Linker"></div><div class="field"><label>Exciting description</label><input name="description" maxlength="180" required placeholder="Land three full lines this week"></div><div class="coach-challenge-fields"><div class="field"><label>Sheet list</label><select name="category">${["daily","one_bang","dialled","lines","percentage","foam_pit","bonus"].map((category) => `<option value="${category}">${escapeHtml(categoryInfo[category]?.label || category)}</option>`).join("")}</select></div><div class="field"><label>Target</label><input name="targetCount" type="number" min="1" max="50" value="3" required></div><div class="field"><label>Who sees it</label><select name="audienceGroup"><option value="">All riders</option>${coachGroups.map(([id,label]) => `<option value="${id}">${escapeHtml(label)}</option>`).join("")}</select></div></div><div class="battle-intro-actions"><button class="primary-btn" type="submit">Publish challenge</button><button class="secondary-btn" type="button" data-close-challenge-builder>Cancel</button></div></form></section>`;
  document.body.append(backdrop);
  backdrop.querySelector("[data-close-challenge-builder]")?.addEventListener("click", () => backdrop.remove());
  backdrop.addEventListener("click", (event) => { if (event.target === backdrop) backdrop.remove(); });
  backdrop.querySelector("#weekly-challenge-builder-form")?.addEventListener("submit", async (event) => {
    event.preventDefault(); const form = new FormData(event.currentTarget); const button = event.currentTarget.querySelector("button[type='submit']"); const restore = setButtonBusy(button,"Publishing...");
    const { error } = await client.rpc("create_weekly_challenge", { p_title:String(form.get("title")||""), p_description:String(form.get("description")||""), p_category:String(form.get("category")||"lines"), p_target_count:Number(form.get("targetCount")||3), p_audience_group:String(form.get("audienceGroup")||"") || null });
    restore(); if (error) return notify(messageFrom(error),"error"); backdrop.remove(); notify("Weekly challenge is live for the selected riders."); await refresh();
  });
}
...
function coachSharedEventsSummaryHtml(events = [], attendance = []) {
  const eventCardHtml = (item, extraClass = "") => {
    const attendees = contestEventAttendees(attendance, item.id);
    const coachGoing = attendees.some((row) => row.athlete_id === state.user?.id);
    return `<article class="contest-event-card ${coachGoing ? "is-going" : ""} ${extraClass}">
  ...
  const featuredRows = events.slice(0, 4).map((item, index) => eventCardHtml(item, index === 3 ? "command-event-desktop-only" : "")).join("");
  const phoneOverflow = events.length > 3
    ? `<details class="command-event-mobile-overflow"><summary><span><strong class="command-event-more-closed">SHOW ${events.length - 3} MORE EVENT${events.length - 3 === 1 ? "" : "S"}</strong><strong class="command-event-more-open">HIDE ADDITIONAL EVENTS</strong><small class="command-event-more-closed">Tap to expand</small><small class="command-event-more-open">Tap to collapse</small></span><b aria-hidden="true">⌄</b></summary><div class="contest-event-grid command-event-overflow-grid">${events.slice(3).map((item) => eventCardHtml(item)).join("")}</div></details>`
    : "";
  return `<div class="coach-shared-events-summary"><div class="contest-event-grid command-event-grid">${featuredRows || `<div class="contest-empty"><strong>No upcoming events yet</strong><span>Open Events & Runs to add the first shared crew event.</span></div>`}</div>${phoneOverflow}<button class="primary-btn full-width-btn" type="button" data-view="contests">VIEW ALL UPCOMING EVENTS</button></div>`;
}
...
function sessionViewerRiderCountersHtml(entry) {
  const categories = [
    ["one_bang", "One Bangs"],
    ["dialled", "Dialled"],
...
async function renderSessionViewer({ forceParkKing = false } = {}) {
  ...
  const [roster, activeGroupSession, battleResult] = await Promise.all([
  ...
  const groupRiderIds = new Set(groupRoster.map((athlete) => athlete.id));
  const groupBattles = (Array.isArray(battleResult.data) ? battleResult.data : []).filter((battle) => ["pending", "accepted"].includes(battle.status) && (battle.participants || []).some((participant) => groupRiderIds.has(participant.athlete_id)));
...
function runTimedPosition(timing, progress) {
  const total = timing.reduce((sum, point) => sum + point.hold + point.travel, 0);
  let elapsed = Math.max(0, Math.min(1, progress)) * total;
  for (let index = 0; index < timing.length; index++) {
    if (elapsed < timing[index].hold) return index;
    elapsed -= timing[index].hold;
    if (elapsed < timing[index].travel) return index + elapsed / timing[index].travel;
    elapsed -= timing[index].travel;
  }
  return Math.max(0, timing.length - 1);
}

let runUndoStack = [], runRedoStack = [];
function rememberRunEdit() {
  if (!state.runBuilder) return;
  const snapshot = JSON.stringify({ ...state.runBuilder, ...currentRunFormState() });
  if (runUndoStack.at(-1) !== snapshot) runUndoStack.push(snapshot);
  if (runUndoStack.length > 60) runUndoStack.shift();
  runRedoStack = [];
  document.querySelector('[data-run-history="undo"]')?.removeAttribute("disabled");
  document.querySelector('[data-run-history="redo"]')?.setAttribute("disabled", "");
}

async function restoreRunEdit(event) {
  const undo = event.currentTarget.dataset.runHistory === "undo";
  const source = undo ? runUndoStack : runRedoStack;
  const destination = undo ? runRedoStack : runUndoStack;
  if (!source.length) return;
  destination.push(JSON.stringify({ ...state.runBuilder, ...currentRunFormState() }));
  state.runBuilder = JSON.parse(source.pop());
...
function updateRunTiming(event) {
  ...
  if (summary) summary.textContent = `${total} seconds planned · ${total <= limit ? `${Math.round((limit-total)*10)/10} seconds remaining` : `${Math.round((total-limit)*10)/10} seconds over limit`}`;
  document.querySelectorAll("#run-builder-live [data-run-map-preview]").forEach(preview => {
    preview.dataset.runTiming = JSON.stringify(runTiming(points));
  });
...
function runPlaybackDefaultSeconds(points = []) {
  return Math.max(1, Math.round(runTiming(points).reduce((sum, point) => sum + point.hold + point.travel, 0) * 10) / 10);
}
...
function runBuilderStage(builder = state.runBuilder) {
  const stage = String(builder?.stage || "route");
  return ["route", "tricks", "playback"].includes(stage) ? stage : "route";
}
...
function runBuilderStepsHtml(stage = "route", pointCount = 0) {
  ...
  return `<nav class="run-builder-steps" aria-label="Run builder steps">${stages.map(([id, number, title, detail], index) => { const unavailable = index > activeIndex || (id !== "route" && !pointCount); return `<button class="${id === stage ? "active" : ""} ${index < activeIndex ? "complete" : ""}" type="button" data-run-builder-stage="${id}" ${unavailable ? "disabled" : ""} ${id === stage ? `aria-current="step"` : ""}><b>${number}</b><span><strong>${title}</strong><small>${detail}</small></span></button>`; }).join("")}</nav>`;
}
...
function runBuilderPanel(runs = [], options = {}) {
  ...
  const stageCopy = {
    ...
    tricks: "Step 2 of 3 · Add optional trick names and timing. Blank dots show NO TRICK in playback.",
  ...
  const body = `${state.profile?.role === "athlete" ? `<div class="actions"><button type="button" class="secondary-btn compact-btn" data-rider-saved-runs="all">Saved runs · yours + coach’s</button></div>` : ""}<div data-live-run-bar>${liveRunBarHtml()}</div><form id="run-builder-form" class="run-builder-form">
...
async function getWeeklyAssignmentPlan(athleteId, targetWeekStart = nextWeekStartDate()) {
  const { data, error } = await client.from("weekly_assignment_plans")
    ...
    .in("status", plannerEditableStatuses)
    .order("sort_order", { ascending: true });
  if (error) throw error;
  const rows = data || [];
  const scheduled = rows.filter((row) => row.status === "scheduled_next_week");
  if (scheduled.length) return scheduled;
  const published = rows.filter((row) => row.status === "published");
  if (published.length) return published;
  const draft = rows.filter((row) => row.status === "draft_next_week");
...
function assignmentDraftKey(assignment = {}) {
  return [
    ...
    assignment.category === "daily" ? venueKey(assignment.venue).toLowerCase() : "",
    String(assignment.trick_name || "").trim().toLowerCase(),
  ].join("|");
}

function completedKeysFromPlannerSource(assignments = [], progressRows = [], attemptRows = []) {
  const byAssignmentId = new Map(assignments.map((assignment) => [assignment.id, assignment]));
  const completedIds = new Set();
  (progressRows || []).forEach((progress) => {
    if (progress?.completed_at) completedIds.add(progress.assignment_id);
  });
  (attemptRows || []).forEach((attempt) => {
    if (attempt?.assignment_id) completedIds.add(attempt.assignment_id);
  });
  const completedKeys = new Set();
  completedIds.forEach((assignmentId) => {
    const assignment = byAssignmentId.get(assignmentId);
    if (assignment) completedKeys.add(assignmentDraftKey(assignment));
  });
  return completedKeys;
}

function withPlannerCompletionFlags(assignments = [], completedKeys = new Set()) {
  return assignments.map((assignment) => ({
    ...assignment,
    completed_this_week: completedKeys.has(assignmentDraftKey(assignment)),
  }));
}

function plannerCompletedStrip(assignments = [], show = false) {
  if (!show) return "";
  const completed = (assignments || []).filter((assignment) => assignment.completed_this_week);
  if (!completed.length) {
    return `<div class="planner-completed-strip empty">No completed tricks in this section yet this week.</div>`;
  }
...
async function getPlannerTemplateAssignments(athleteId, targetWeekStart = nextWeekStartDate()) {
  ...
  const { data: currentRows, error: currentError } = await client.from("weekly_trick_assignments")
    .select("*")
    .eq("athlete_id", athleteId)
  ...
  let rows = (currentRows || []).filter((assignment) => categoryInfo[assignment.category]);
  ...
  if (rows.length) {
    ...
    const [progressResult, attemptsResult] = await Promise.all([
      client.from("assignment_progress").select("assignment_id, completed_at, progress_date").in("assignment_id", assignmentIds),
      client.from("assignment_attempts").select("assignment_id").eq("athlete_id", athleteId).eq("week_start", currentWeek).in("assignment_id", assignmentIds),
    ]);
    if (progressResult.error) throw progressResult.error;
    if (attemptsResult.error) throw attemptsResult.error;
    completedKeys = completedKeysFromPlannerSource(rows, progressResult.data || [], attemptsResult.data || []);
  } else {
    const { data, error } = await client.from("weekly_trick_assignments")
      ...
      .limit(300);
    if (error) throw error;
    rows = (data || []).filter((assignment) => categoryInfo[assignment.category]);
  }

  if (!rows.length) return cacheSet(cacheKey, { assignments: [], completedKeys: [] });
  ...
  const assignments = rows
    ...
    .map((assignment, index) => ({
      ...
      source_week_start: assignment.week_start,
      completed_this_week: completedKeys.has(assignmentDraftKey(assignment)),
      sort_order: Number.isFinite(Number(assignment.sort_order)) ? Number(assignment.sort_order) : index,
    }));
  return cacheSet(cacheKey, {
  ...
  document.querySelector("#view").innerHTML = `
    ...
    <section class="panel planner-hero">
      <div class="planner-hero-copy">
        ...
        <p>Choose a rider, start from their current active list, then schedule the edited version for next week.</p>
      </div>
...
function coachPreviewTabs(mode = "student") {
  return mode === "parent"
    ...
    : [["home", "Home"], ["session", "Session"], ["challenges", "Challenges"], ["contests", "Contests"], ["board", "Board"], ["profile", "Profile"]];
}
...
function coachPreviewPageHtml(context) {
  ...
  if (activeTab === "challenges") return coachStudentPreviewChallengesHtml(context);
...
function coachStudentPreviewChallengesHtml({ assignments }) {
  const completedLines = (assignments || []).filter((assignment) => assignment.category === "lines" && isAssignmentComplete(assignment)).length;
  const progress = Math.min(3, completedLines);
  return `<div class="phone-preview-content">
    <section class="panel weekly-challenge-card"><div class="panel-head"><div><div class="panel-title">Line Linker</div><div class="panel-meta">Weekly challenge · resets Sunday</div></div><span class="pill">${progress}/3</span></div><h2>Complete 3 Lines this week</h2><div class="xp-bar"><span style="width:${Math.round((progress / 3) * 100)}%"></span></div></section>
    <section class="panel"><div class="panel-head"><div><div class="panel-title">Weekly rider battles</div><div class="panel-meta">Riders can invite one another to compare weekly points</div></div></div><div class="empty compact-empty">Battle requests and live scores appear here.</div></section>
  </div>`;
}
...
function coachPreviewTricktionaryHtml({ athlete, tricktionaryData }) {
  ...
  const entries = landedTricktionaryEntries(data).map((entry) => ({ ...entry, ownerId: athlete.id }));
  return `<div class="phone-preview-content">
    <section class="panel"><div class="panel-head"><div><div class="panel-title">Tricktionary</div><div class="panel-meta">${entries.length} landed tricks · Daily PB ${formatPbTime(data.profile?.daily_pb_seconds)}</div></div></div>${tricktionaryBoardHtml(entries, data.attempts || [], { profile: data.profile })}</section>
    <section class="panel"><div class="panel-head"><div><div class="panel-title">Attempted this week</div></div></div>${weeklyAttemptsHtml((data.attempts || []).filter((attempt) => attempt.week_start === weekStartDate()), data.profile)}</section>
  </div>`;
}
...
function coachStudentPreviewProfileHtml({ athlete, weeklyRow }) {
  ...
  return `<div class="phone-preview-content">
    ...
    ${goalsReadonlyHtml(athlete)}
    <section class="panel profile-library-card"><div class="panel-title">My Tricktionary</div><p class="subcopy">The rider opens their landed trick library from Profile & Settings.</p></section>
  </div>`;
}
...
function coachParentPreviewHtml({ athlete, assignments, awards, assignmentAttempts = [], dashboardItems, sessions, runs, visibleFeedback, rank, weeklyRow, weeklyPercent, completedWeekly, weeklyItems, xpSummary }) {
  ...
  return `<div class="phone-preview-content">
    <section class="panel parent-child-card">
      ...
      <div class="settings-divider"></div>
      <div class="panel-title">Attempted this week</div>
      ${weeklyAttemptsHtml(assignmentAttempts, athlete)}
      ${goalsReadonlyHtml(athlete)}
      <div class="settings-divider"></div>
...
async function renderStudentProfile() {
  ...
  view.innerHTML = `
    ...
    <section class="panel xp-coach-panel">
      ...
      <div class="settings-divider"></div>
      ${levelBadgesAccordionHtml(scoreXp, { title: "XP Badges", meta: "Unlocked by lifetime XP · coach view" })}
      <div class="settings-divider"></div>
      <div class="panel-head"><div><div class="panel-title">XP adjustment</div><div class="panel-meta">Admin-only control for the rider's visible level and badge progress</div></div></div>
  ...
  document.querySelector("#xp-adjust-form")?.addEventListener("submit", saveCoachXpAdjustment);
...
async function saveCoachXpAdjustment(event) {
  event.preventDefault();
  if (!isCoachRole(state.profile?.role)) return notify("Only coaches can adjust XP.", "error");
  const form = new FormData(event.currentTarget);
  const amount = Number(form.get("amount") || 0);
  const action = String(form.get("action") || "add");
  const reason = String(form.get("reason") || "").trim();
  if (!amount || amount < 1) return notify("Enter an XP amount first.", "error");
  if (action === "deduct" && !reason) return notify("Add a reason before deducting XP.", "error");
  const signedXp = action === "deduct" ? -Math.abs(amount) : Math.abs(amount);
  const button = event.currentTarget.querySelector("button");
  button.disabled = true;
  button.textContent = "Saving...";
  const { data, error } = await client.rpc("adjust_athlete_xp", {
    p_athlete_id: state.selectedAthleteId,
    p_xp: signedXp,
    p_reason: reason,
  });
  if (error) {
    button.disabled = false;
    button.textContent = "Apply XP Change";
    return notify(messageFrom(error), "error");
  }
  const result = Array.isArray(data) ? data[0] : data;
  notify(`${signedXp > 0 ? "Added" : "Deducted"} ${Math.abs(signedXp)} XP. Level ${result?.level_after || result?.level || "updated"}.`);
  await renderStudentProfile();
}
...
async function removeCoachManualTrick(event) {
  ...
  const { error } = await client.rpc("remove_manual_tricktionary_entry", { p_athlete_id: athlete.id, p_entry_id: removeId });
  if (error) return notify(messageFrom(error), "error");
  notify("Manual Tricktionary trick removed. Historical session data was kept.");
  await renderStudentProfile();
}
...
function bindRunBuilderActions(root = document) {
  ...
  root.querySelectorAll("[data-run-builder-stage]").forEach((button) => button.addEventListener("click", setRunBuilderStage));
...
async function setRunBuilderStage(event) {
  event.preventDefault();
  const stage = String(event.currentTarget.dataset.runBuilderStage || "route");
  const stageOrder = ["route", "tricks", "playback"];
  if (!stageOrder.includes(stage)) return;
  if (stage !== "route" && !state.runBuilder?.points?.length) return notify("Draw at least one route point first.", "error");
  const currentStageIndex = stageOrder.indexOf(runBuilderStage());
  if (stageOrder.indexOf(stage) > currentStageIndex + 1) return;
  stopRunPlayback();
  state.runBuilder = { ...currentRunFormState(), stage };
  if (stage !== "route") state.runBuilder.selectedPointIndex = -1;
  else if (state.runBuilder.points?.length && Number(state.runBuilder.selectedPointIndex) < 0) state.runBuilder.selectedPointIndex = state.runBuilder.points.length - 1;
  await runBuilderRefreshView();
}

function updateRunBuilderTrick(event) {
  const index = Number(event.currentTarget.dataset.runTrickIndex);
  if (!state.runBuilder?.points?.[index]) return;
  rememberRunEdit();
  const label = event.currentTarget.value.slice(0, 80);
  state.runBuilder.points[index] = { ...state.runBuilder.points[index], label };
  const marker = document.querySelector(`#run-map [data-run-point-number="${index + 1}"]`);
  if (marker) {
    marker.setAttribute("aria-label", `${index + 1}. ${label || `Point ${index + 1}`}`);
    marker.dataset.runPointLabel = label.trim() || "NO TRICK";
  }
}
...
function focusRunBuilderTrick(event) {
  ...
  document.querySelectorAll("#run-map .run-marker").forEach((marker) => marker.classList.toggle("is-selected", Number(marker.dataset.runPointNumber) === index + 1));
}
...
function updateRunBuilderMapDom(changedIndex = null) {
  ...
  const finish = points.length > 1 ? points[points.length - 1] : null;
...
function startRunPointDrag(event) {
  ...
  state.runPointDragStart = { x: event.clientX, y: event.clientY };
  state.runPointMapClickBlockUntil = performance.now() + 900;
  state.runPointDragMoved = false;
  state.runPointDragNeedsSelectionRefresh = Number(document.querySelector("[data-selected-run-point]")?.dataset.selectedRunPoint) !== state.draggedRunPoint;
  state.runPointDragPending = null;
  document.querySelector("#run-map")?.classList.add("is-dragging");
  ...
  document.addEventListener("pointermove", dragRunPoint, { passive: false });
  document.addEventListener("pointerup", stopRunPointDrag, { once: true });
  document.addEventListener("pointercancel", stopRunPointDrag, { once: true });
}
...
function dragRunPoint(event) {
  ...
  const start = state.runPointDragStart;
  if (start && Math.hypot(event.clientX - start.x, event.clientY - start.y) > 3) state.runPointDragMoved = true;
  state.runPointDragPending = { index, x, y };
  if (state.runPointDragFrame) return;
  state.runPointDragFrame = requestAnimationFrame(() => {
    state.runPointDragFrame = null;
    const pending = state.runPointDragPending;
    state.runPointDragPending = null;
    if (!pending || !state.runBuilder?.points?.[pending.index]) return;
    state.runBuilder.points[pending.index] = { ...state.runBuilder.points[pending.index], x: pending.x, y: pending.y };
    updateRunBuilderMapDom(pending.index);
  });
}
...
function stopRunPointDrag() {
  ...
  if (state.runPointDragFrame) cancelAnimationFrame(state.runPointDragFrame);
  state.runPointDragFrame = null;
  const pending = state.runPointDragPending;
  if (pending && state.runBuilder?.points?.[pending.index]) {
    state.runBuilder.points[pending.index] = { ...state.runBuilder.points[pending.index], x: pending.x, y: pending.y };
    updateRunBuilderMapDom(pending.index);
  }
  if (state.runPointDragMoved) state.runPointDragClickBlockUntil = performance.now() + 300;
  // Browsers can retarget the click following any marker gesture to the map,
  // including a long press with almost no movement. Always block that map click.
  state.runPointMapClickBlockUntil = performance.now() + 900;
  document.querySelector("#run-map")?.classList.remove("is-dragging");
  document.querySelectorAll("[data-run-point-index].is-dragging").forEach((marker) => marker.classList.remove("is-dragging"));
  state.runPointDragPending = null;
  state.runPointDragStart = null;
  state.runPointDragMoved = false;
...
async function selectRunPoint(event) {
  event.preventDefault();
  event.stopPropagation();
  if (performance.now() < state.runPointDragClickBlockUntil) return;
  const index = Number(event.currentTarget.dataset.selectRunPoint);
  if (!state.runBuilder?.points?.[index]) return;
  if (Number(state.runBuilder.selectedPointIndex) === index) return;
...
function updateSelectedRunPoint(event) {
  ...
  if (marker) {
    ...
    marker.dataset.runPointLabel = String(point.label || "").trim() || "NO TRICK";
  }
  updateRunBuilderMapDom(index);
}
...
function paintRunPlayback(controls, progress = 0) {
  ...
  const timing = JSON.parse(preview.dataset.runTiming || "null");
  const routePosition = timing ? runTimedPosition(timing, safeProgress) : markers.length > 1 ? safeProgress * (markers.length - 1) : 0;
  preview.classList.toggle("is-playback-highlighted", safeProgress > 0);
...
async function playFinishedRunBuilder() {
  ...
  state.runBuilder = { ...currentRunFormState(), stage: "playback", selectedPointIndex: -1 };
  await runBuilderRefreshView();
  ...
  if (!playButton) return notify("The run playback could not open. Please try again.", "error");
...
async function clearRunBuilder() {
  rememberRunEdit();
  stopRunPlayback();
  state.runBuilder = { ...currentRunFormState(), points: [], stage: "route" };
...
async function editRunPlan(event) {
  const button = event.currentTarget;
  if (button.disabled) return;
  if (!liveRun && state.runBuilder?.points?.length && !window.confirm("Your current run has unsaved changes. Open this saved run instead?")) return;
  if (liveRun && !await leaveLiveRun()) return;
