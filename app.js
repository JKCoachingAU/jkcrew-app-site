...
function runBuilderPanel(runs = [], options = {}) {
  ...
  return `<section class="panel run-builder-live ${builder.fullscreenEditor ? "run-builder-fullscreen-editor" : ""}" id="run-builder-live"><div class="panel-head"><div><div class="eyebrow">${builder.athleteName ? `Private plan for ${escapeHtml(builder.athleteName)}` : "Build mode"}</div><div class="panel-title">Contest Run Planner</div><div class="panel-meta">Tap the park, bend the coloured route, finish on any number, then control the full playback up to 60 seconds</div></div>${builder.fullscreenEditor ? `<button class="secondary-btn compact-btn" id="exit-run-builder-fullscreen" type="button">Exit full screen</button>` : options.live ? `<button class="secondary-btn compact-btn" id="close-run-builder-top" type="button">Close</button>` : ""}</div>
    ${body}
  </section>`;
}
...
function bindRunBuilderActions() {
  document.documentElement.classList.toggle("run-builder-fullscreen-open", Boolean(state.runBuilder?.fullscreenEditor));
  ...
  document.querySelector("#exit-run-builder-fullscreen")?.addEventListener("click", exitRunBuilderFullscreen);
...
function currentRunFormState() {
  ...
  return {
    ...
    fullscreenEditor: Boolean(state.runBuilder?.fullscreenEditor),
  };
}
...
async function addRunBuilderPoint(event) {
  ...
  const isFirstPoint = state.runBuilder.points.length === 0;
  ...
  if (isFirstPoint) enterRunBuilderFullscreen();
  await runBuilderRefreshView();
}

function enterRunBuilderFullscreen() {
  if (!state.runBuilder) return;
  state.runBuilder.fullscreenEditor = true;
  document.documentElement.classList.add("run-builder-fullscreen-open");
  const page = document.documentElement;
  try {
    const fullscreenRequest = page.requestFullscreen?.({ navigationUI: "hide" }) || page.webkitRequestFullscreen?.();
    ...
    // The fixed full-viewport editor remains available when device fullscreen is blocked.
  }
}

async function leaveRunBuilderFullscreen() {
  document.documentElement.classList.remove("run-builder-fullscreen-open");
  try { screen.orientation?.unlock?.(); } catch (_error) { /* Device does not expose orientation controls. */ }
  const activeFullscreen = document.fullscreenElement || document.webkitFullscreenElement;
  if (activeFullscreen === document.documentElement) {
    try {
      if (document.exitFullscreen) await document.exitFullscreen();
      else if (document.webkitExitFullscreen) document.webkitExitFullscreen();
    } catch (_error) { /* The fixed editor has already closed. */ }
  }
}

async function exitRunBuilderFullscreen() {
  if (!state.runBuilder) return;
  state.runBuilder = { ...currentRunFormState(), fullscreenEditor: false };
  await leaveRunBuilderFullscreen();
  await runBuilderRefreshView();
  document.querySelector("#run-builder-live")?.scrollIntoView({ behavior: "smooth", block: "start" });
}
