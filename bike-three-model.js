...
/** Original, dimensioned 20-inch BMX mesh. Geometry, materials and view are separate from saved config. */
export function createBike(configuration) {
  ...
  for(const [key,color] of Object.entries(c.colors)){
    ...
    paints[key]=finish==='chrome'?mat({color:'#e6ebf0',metalness:1,roughness:.095,envMapIntensity:1.12}):
      finish==='raw'?mat({color:'#969da1',metalness:1,roughness:.38,envMapIntensity:.9}):
      ...
      mat({color,metalness:metalParts.has(key)?.42:.025,roughness:finish==='matte'?.64:(key==='seat'?.66:.29),clearcoat:finish==='matte'?0:metalParts.has(key)?.8:.12,clearcoatRoughness:.18});
  }
  const tyres=c.tyreStyle==='white'?whiteRubber:rubber;
  ...
  const rear=V(-.54,.266),front=V(.505,.266),bb=V(-.16,.295),seat=V(-.255,.61),headLow=V(.355,.58),headTop=V(.324,.703);
  // Closed, double-wall rims, full-volume tyres, individual three-cross spokes and spoke nipples.
  for(const [wheel,p] of [['rear',rear],['front',front]]){
    ...
    for(const s of [-1,1]){
      cylinder('hubs',p.clone().add(V(0,0,s*.032)),p.clone().add(V(0,0,s*.036)),.027,paints.hubs);
    ...
    const tread=[];const tmp=new T.Object3D();
    for(let i=0;i<100;i++)for(const stripe of [-1,0,1]){
      const a=i/100*TAU+(stripe%2)*.013;
      tmp.position.copy(p).add(V(Math.cos(a)*(.264-Math.abs(stripe)*.004),Math.sin(a)*(.264-Math.abs(stripe)*.004),stripe*.017));
      tmp.rotation.set(0,0,a-Math.PI/2);tmp.rotateY(stripe*.23);tmp.scale.set(.010,.0019,.009);tmp.updateMatrix();tread.push(tmp.matrix.clone());
    }
    batch('tyres',new T.BoxGeometry(1,1,1),tyres,tread);
  }
  ...
  // Fork steerer and gently curved tapered fork legs.
  ...
  for(const s of [-1,1]){
    ...
    const delta=end.clone().sub(start),curve=new T.CatmullRomCurve3([start,start.clone().lerp(end,.58).add(V(-.005,0,0)),end]);
    const g=new T.TubeGeometry(curve,32,.016,14,false),positions=g.attributes.position;
    for(let j=0;j<=32;j++){const centre=curve.getPointAt(j/32),scale=1-j/32*.27;for(let k=0;k<=14;k++){const i=j*15+k,p=V().fromBufferAttribute(positions,i).sub(centre).multiplyScalar(scale).add(centre);positions.setXYZ(i,p.x,p.y,p.z);}}g.computeVertexNormals();add('fork',g,paints.fork);
    cylinder('fork',end.clone().add(V(0,0,-.004)),end.clone().add(V(0,0,.004)),.016,paints.fork);
  }
  cylinder('fork',crown.clone().add(V(0,0,-.035)),crown.clone().add(V(0,0,.035)),.025,paints.fork);
  ...
  // Seat tube follows the actual seat-tube axis, with clamp, rails and sewn edge.
  const saddle=V(-.285,.728,0),seatAxis=seat.clone().sub(bb).normalize();
  ...
  for(const s of [-1,1])tube('seat',[V(-.365,.726,s*.020),V(-.306,.712,s*.025),V(-.239,.714,s*.020),V(-.193,.737,s*.01)],.0029,steel,20);
  const seatHeight=c.seatStyle==='padded'?.048:.035;
  const slices=[[-.414,.002],[-.405,.031],[-.391,.057],[-.365,.071],[-.334,.069],[-.302,.059],[-.270,.045],[-.237,.029],[-.204,.024],[-.176,.022],[-.163,.012],[-.159,.002]];
  ...
  for(let j=0;j<slices.length;j++)for(let i=0;i<=steps;i++){
    ...
    const dome=Math.max(0,Math.sin(angle))*Math.sin(j/(slices.length-1)*Math.PI)*.009;pos.push(x,.749+lift+Math.sin(angle)*seatHeight*.5+dome,Math.cos(angle)*w);
  ...
  for(const j of [0,slices.length-1]){const centre=pos.length/3,[x]=slices[j];pos.push(x,.749+(x+.3)*.025,0);uv.push(j/(slices.length-1),.5);for(let i=0;i<steps;i++){const a=j*(steps+1)+i;indices.push(centre,...(j===0?[a+1,a]:[a,a+1]));}}
  ...
  for(const s of [-1,1]){const seam=plain(c.colors.seat,.8);seatSeamMaterials.push(seam);tube('seat',slices.map(([x,w])=>V(x,.749+(x+.3)*.025-.007,s*w*.99)),.0011,seam,36);}
  ...
  // Tall BMX bars: actual swept tubing, crossbar and grip ends. Both styles are structural variants.
  const barX=barCentre.x,barY=barCentre.y,gripY=barY+.210;
  if(c.barStyle==='two-piece'){
    tube('bars',[V(barX-.067,gripY,-.335),V(barX-.055,gripY-.003,-.175),V(barX-.030,gripY-.022,-.113),V(barX,barY+.054,-.076),V(barX,barY,-.04),V(barX,barY,.04),V(barX,barY+.054,.076),V(barX-.030,gripY-.022,.113),V(barX-.055,gripY-.003,.175),V(barX-.067,gripY,.335)],.0111,paints.bars,100);
    tube('bars',[V(barX-.013,barY+.138,-.100),V(barX-.017,barY+.142,0),V(barX-.013,barY+.138,.100)],.008,paints.bars,20);
  }else{
    cylinder('bars',V(barX,barY,-.065),V(barX,barY,.065),.0111,paints.bars);
    for(const s of [-1,1]){
      tube('bars',[V(barX,barY,s*.06),V(barX-.012,barY+.108,s*.085),V(barX-.034,gripY-.018,s*.105),V(barX-.055,gripY,s*.158),V(barX-.067,gripY,s*.335)],.0111,paints.bars,48);
    }
    cylinder('bars',V(barX-.020,barY+.155,-.096),V(barX-.020,barY+.155,.096),.008,paints.bars);
  }
  const gripRings=[];
  for(const s of [-1,1]){
    const a=V(barX-.065,gripY,s*.204),b=V(barX-.068,gripY,s*.347);
    cylinder('grips',a,b,.016,paints.grips);
    cylinder('grips',a.clone().add(V(0,0,-s*.004)),a,.020,paints.grips);
  ...
  // Strong three-piece cranks with level open platform pedals, steel spindles and traction pins.
  for(const s of [-1,1]){
    ...
    cylinder('cranks',p,end,.014,paints.cranks,.011);sphere('cranks',p,.016,paints.cranks);sphere('cranks',end,.012,paints.cranks);bolt('cranks',p.clone().add(V(0,0,s*.015)),V(0,0,s),.008);
  ...
  for(const which of ['front','rear'])if(c.brakeStyle===which||c.brakeStyle==='dual'){
    ...
    for(const s of [-1,1]){
      ...
      box('brakes',pivotSide.clone().add(V(.002,-.014,-s*.016)),.023,.007,.009,rubber);
    }
    tube('brakes',[pivot.clone().add(V(-.015,.049,-.027)),pivot.clone().add(V(-.012,.067,0)),pivot.clone().add(V(-.015,.049,.027))],.0008,steel,16);
    const s=which==='front'?-1:1,lever=V(barX-.067,gripY-.013,s*.185);
    ...
    tube('brakes',[lever, V(barX+.088,gripY-.09,s*.09), V(headTop.x+.08,headTop.y-.10,s*.042), which==='front'?pivot.clone().add(V(.014,.115,0)):seat.clone().add(V(.06,.041,.023)),pivot.clone().add(V(-.012,.067,0))],.002,blackPlastic,64);
  }
