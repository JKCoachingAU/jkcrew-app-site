...
const JKCrewBikeGarage = (() => {
  ...
  const icon = paths => `<svg class="bike-ui-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.65" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
  const groupIcons = {
    Frame:icon('<path d="m14 3 7 7-10 10H4v-7zM11 6l7 7M4 17l3 3"/>'),
    'Front end':icon('<path d="M3 5h3l3 11h6l3-11h3M8 12h8M12 16v5M9 21h6"/>'),
    Wheels:icon('<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="2"/><path d="m6 6 4.5 4.5m3 3L18 18M6 18l4.5-4.5m3-3L18 6"/>'),
    Details:icon('<path d="m14 4 6 6-10 10H4v-6zM11 7l6 6M16 2l6 6"/>')
  };
  const toolIcons = {
    undo:icon('<path d="M9 5 4 10l5 5M4 10h10a6 6 0 0 1 0 12" transform="translate(0 -2)"/>'),
    redo:icon('<path d="m15 5 5 5-5 5m5-5H10a6 6 0 0 0 0 12" transform="translate(0 -2)"/>'),
    shuffle:icon('<path d="m17 3 4 4-4 4m0 2 4 4-4 4M3 7h3c5 0 7 10 12 10h3M3 17h3c2 0 4-2 5-5m3-4 4-1h3"/>'),
    camera:icon('<circle cx="12" cy="12" r="7"/><path d="M12 2v5m0 10v5M2 12h5m10 0h5"/>'),
    copy:icon('<rect x="8" y="8" width="12" height="12" rx="2"/><path d="M16 8V4H4v12h4"/>'),
    back:icon('<path d="m10 5-7 7 7 7M3 12h18"/>'),
  ...
  function mount({ root, client, userId, isCurrent, onBack }) {
    ...
    function retry3D() {
      ...
      artwork.innerHTML='';artwork.dataset.view='3d';
    ...
    function controls() {
      ...
      scrollArea.innerHTML=`
        ...
        ${usesColour?`<div class="bike-colour-section"><div class="bike-colour-heading"><span class="bike-eyebrow">${part==='frame'&&config.framePaint==='fade'?'FIRST COLOUR':'COLOUR'}</span><span class="bike-colour-name"><i style="background:${selected}"></i>${colourName}</span></div><div class="bike-palette" aria-label="${labels[part]} colour">${palette.map(([label,value])=>`<button type="button" class="bike-swatch" data-bike-colour="${value}" style="--swatch:${value}" title="${label}" aria-label="${label} ${labels[part].toLowerCase()}" aria-pressed="${selected===value}"><span aria-hidden="true">${selected===value?'✓':''}</span></button>`).join('')}</div><label class="bike-custom-colour"><span>Custom colour</span><input type="color" data-bike-custom-colour value="${selected}" aria-label="Custom ${labels[part].toLowerCase()} colour"></label></div>`:''}
    ...
    root.addEventListener('click',event=>{
      ...
      if(button.hasAttribute('data-bike-3d-retry'))return retry3D();
