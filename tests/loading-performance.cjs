...
async function workerChecks() {
 ...
 stores.set(origin+'index.html','cached-shell');stores.set(origin+'app.js?v=2.14.97','cached-js');stores.set(origin+'vendor/supabase-2.116.0.min.js','cached-sdk');
 ...
 const count=networkCalls.length;assert.equal(await dispatch(origin+'app.js?v=2.14.97').result,'cached-js');assert.equal(await dispatch(origin+'vendor/supabase-2.116.0.min.js').result,'cached-sdk');assert.equal(networkCalls.length,count);
 ...
 network=deferred();const missing=dispatch(origin+'styles.css?v=2.14.97');network.resolve({ok:false,status:404});assert.equal((await missing.result).status,404);assert(!stores.has(origin+'styles.css?v=2.14.97'));
 ...
 for(const name of ['studio-hardware-v2.webp','studio-metal-v2.webp']) { network=deferred();const response=dispatch(origin+'images/bike-garage/'+name);network.resolve(photo);assert.equal(await response.result,photo,'New hardware uses the public cache'); }
 assert(!fs.readFileSync(path.join(root,'sw.js'),'utf8').match(/const APP_SHELL = \[([\s\S]*?)\];/)[1].includes('studio-white'),'Optional bike photos must not delay app-shell installation');
}
