...
async function workerChecks() {
 ...
 stores.set(origin+'index.html','cached-shell');stores.set(origin+'app.js?v=2.14.117','cached-js');stores.set(origin+'vendor/supabase-2.116.0.min.js','cached-sdk');
 ...
 const count=networkCalls.length;assert.equal(await dispatch(origin+'app.js?v=2.14.117').result,'cached-js');assert.equal(await dispatch(origin+'vendor/supabase-2.116.0.min.js').result,'cached-sdk');assert.equal(networkCalls.length,count);
 ...
 network=deferred();const missing=dispatch(origin+'styles.css?v=2.14.117');network.resolve({ok:false,status:404});assert.equal((await missing.result).status,404);assert(!stores.has(origin+'styles.css?v=2.14.117'));
 ...
 for(const name of ['bike-three-model.js?v=2.14.117','vendor/three.module.min.js','vendor/three.core.min.js','vendor/OrbitControls.js','vendor/RoomEnvironment.js']) { network=deferred();const response=dispatch(origin+name);network.resolve(photo);assert.equal(await response.result,photo,'3D assets load and cache only on demand'); }
