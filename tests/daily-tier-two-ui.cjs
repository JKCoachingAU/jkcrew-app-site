...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(async()=>{const original=client.rpc;window.failUnlock=true;client.rpc=(name,args)=>name==='unlock_daily_tier_two'&&failUnlock?Promise.resolve({error:{message:'Unlock temporarily unavailable'}}):original(name,args);round.eligible=true;await control.refresh();});
  ok(await page.getByRole('alert').isVisible(),'Confirmed server eligibility exposes unlock errors even without a local session hint');
  await page.evaluate(()=>failUnlock=false);await page.getByRole('button',{name:'Retry'}).click();await page.waitForFunction(()=>control.getState()?.unlocked);
  ok(await page.locator('.daily-tier-two-unlock').isVisible(),'Eligible rider automatically sees the unlock moment');eq(await page.locator('input[type=checkbox]').count(),2,'Fresh list is prepared without another unlock tap');
  await page.locator('[data-tier-two-enter]').click();await page.waitForFunction(()=>changes.some(c=>c.reveal_claimed));eq(await page.locator('input[type=checkbox]').count(),2);ok(await page.locator('.daily-tier-two--reveal').count());
