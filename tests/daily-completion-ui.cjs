...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{holdAction=true;holdConfirm=true;failConfirm=true;});
    ...
    await page.waitForFunction(()=>confirmReleases.length===1);
    assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='confirm_daily_finish').length),1,'The final tick automatically submits its verified finish');
    assert.equal(await page.locator('[data-confirm-daily]:visible,[data-cancel-daily]:visible').count(),0,'A full list has no second Finish or cancel prompt while saving');
    assert.equal(await page.locator('.daily-finish-time strong').textContent(),'01:10','Final tap time excludes network delay');
    await capture(page,'daily-automatic-save-phone');
    const firstCandidate=await page.evaluate(()=>dailyFinishUi.current.candidate.id);
    ...
    await page.evaluate(()=>renderSession());assert.equal(await page.locator('.daily-finish-backdrop').count(),1,'A list refresh does not duplicate the automatic save');
    await page.keyboard.press('Escape');assert.equal(await page.locator('.daily-finish-backdrop').count(),1,'Escape cannot dismiss a pending finish save');
    assert.equal(await page.locator('.daily-result-dialog').count(),0,'No saved receipt appears while saving');
    await page.evaluate(()=>{holdConfirm=false;confirmReleases.splice(0).forEach(release=>release());});
    await page.waitForFunction(()=>document.querySelector('[data-confirm-daily]')?.textContent==='Retry finish');
    assert.equal(await page.locator('.daily-result-dialog').count(),0,'A failed automatic save never celebrates');
    assert.equal(await page.locator('[data-cancel-daily]').textContent(),'Back to session');
    assert((await page.locator('.daily-finish-error').textContent()).includes('same finish'));
    await page.locator('[data-cancel-daily]').click();
    assert.equal(await page.locator('.daily-finish-backdrop').count(),0);
    assert.equal(await page.locator('[data-assignment-id="r1-a1"]').getAttribute('data-assignment-action'),'unlanded','Back to session keeps the landed trick');
    const before=await page.locator('#trick-timer').textContent();await page.evaluate(()=>{clockNow+=5000;updateTimer();});assert.notEqual(await page.locator('#trick-timer').textContent(),before,'A failed save leaves the timer running');
    await page.evaluate(()=>renderSession());assert.equal(await page.locator('.daily-finish-backdrop').count(),0,'Reload does not reopen a failed final popup');
    ...
    await page.evaluate(()=>failConfirm=true);
    await page.locator('[data-assignment-id="r1-a1"]').click();await page.waitForFunction(()=>document.querySelector('[data-confirm-daily]')?.textContent==='Retry finish');
    ...
    await page.locator('[data-cancel-daily]').click();await page.evaluate(()=>{clockNow+=3000;holdPrepare=true;holdConfirm=true;failConfirm=true;});
    await page.locator('#finish-daily-tricks').click();await page.evaluate(()=>{clockNow+=9000;});
    await page.evaluate(()=>{holdPrepare=false;prepareReleases.splice(0).forEach(release=>release());});await page.waitForFunction(()=>confirmReleases.length===1);
    const reopened=await page.evaluate(()=>dailyFinishUi.current.candidate.id);assert.notEqual(reopened,corrected);assert.equal(await page.locator('.daily-finish-time strong').textContent(),'01:30','Manual full finish uses its new Finish tap, excluding wait');
    assert.equal(await page.locator('[data-confirm-daily]:visible,[data-cancel-daily]:visible').count(),0,'Manual full finish also saves without a second confirmation');
    assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='confirm_daily_finish').length),3,'Each new full-list candidate submits automatically');
    ...
    await page.evaluate(()=>holdConfirm=true);await page.locator('[data-confirm-daily]').click();
    await page.evaluate(()=>{void confirmDailyFinish(dailyFinishUi.current);});
    assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='confirm_daily_finish').length),4,'Repeated retry submits only once while saving');
    await page.evaluate(()=>{holdConfirm=false;confirmReleases.splice(0).forEach(release=>release());});await page.waitForSelector('.daily-result-dialog');
    const confirms=await page.evaluate(()=>rpcCalls.filter(row=>row.name==='confirm_daily_finish').map(row=>row.args.p_candidate_id));assert.deepEqual(confirms,[firstCandidate,corrected,reopened,reopened],'Retry preserves the automatic save’s idempotency key');
    ...
    assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='confirm_daily_finish').length),4,'Viewing saved result awards nothing again');
    ...
    await page.locator('#finish-daily-tricks').click();await page.waitForSelector('[data-confirm-daily]');assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='prepare_daily_finish').at(-1).args.p_venue),'');
    await page.locator('[data-cancel-daily]').click();
    await page.locator('[data-assignment-id="r1-default0"]').click();await page.waitForFunction(()=>daily('r1')[0].completed);
    await page.locator('[data-assignment-id="r1-default1"]').click();await page.waitForSelector('.daily-result-dialog');
    assert.equal(await page.evaluate(()=>rpcCalls.filter(row=>row.name==='record_daily_trick_action').at(-1).args.p_venue),'');
    assert.equal(await page.evaluate(()=>results.r1.all_completed),true,'Default list saves automatically after its last trick');
    await page.locator('[data-keep-riding]').click();
    ...
    await page.evaluate(()=>{holdAction=true;holdConfirm=true;});
    await page.locator('[data-assignment-id="r1-a1"]').click();
    await page.locator('[data-viewer-athlete="r2"]').click();await page.locator('[data-assignment-id="r2-a1"]').click();
    await page.evaluate(()=>{holdAction=false;actionReleases.splice(0).forEach(release=>release());});await page.waitForFunction(()=>confirmReleases.length===1&&dailyFinishUi.queued.length===1);
    assert.equal(await page.locator('.daily-finish-backdrop').count(),1,'Concurrent riders save through one active dialog at a time');
    assert.equal(await page.locator('[data-confirm-daily]:visible,[data-cancel-daily]:visible').count(),0,'Coach full completion has no second Finish prompt');
    assert((await page.locator('.daily-finish-dialog .eyebrow').textContent()).includes('Test Rider One'));
    await page.evaluate(()=>confirmReleases.splice(0).forEach(release=>release()));await page.waitForSelector('.daily-result-dialog');assert((await page.locator('.daily-result-dialog').textContent()).includes('Test Rider One'));
    assert.equal(await page.evaluate(()=>state.sessionViewerActiveSessionCache.coach_group_session_participants[0].daily_finish_seconds),70);assert.equal(await page.evaluate(()=>state.sessionViewerActiveSessionCache.coach_group_session_participants[1].daily_finish_seconds),null,'Other rider stays unfinished until their queued save');
    await page.locator('[data-keep-riding]').click();await page.waitForFunction(()=>confirmReleases.length===1);
    assert((await page.locator('.daily-finish-dialog .eyebrow').textContent()).includes('Test Rider Two'));
    assert.equal(await page.locator('[data-confirm-daily]:visible,[data-cancel-daily]:visible').count(),0,'A queued full finish starts saving automatically');
    await page.evaluate(()=>{holdConfirm=false;confirmReleases.splice(0).forEach(release=>release());});await page.waitForSelector('.daily-result-dialog');
    assert.equal(await page.evaluate(()=>sessions.r2.daily_completed_seconds),70);assert.equal(await page.evaluate(()=>state.sessionViewerActiveSessionCache.status),'active','Group clock stays active');
    ...
    await page.evaluate(()=>{holdConfirm=true;failConfirm=true;});
    await page.locator('[data-finish-daily-athlete="r2"]').click();await page.waitForFunction(()=>confirmReleases.length===1);
    await page.evaluate(()=>{state.view='coachHome';dismissDailyFinishForNavigation();state.view='sessionViewer';holdConfirm=false;confirmReleases.splice(0).forEach(release=>release());});
    await page.waitForFunction(()=>dailyFinishUi.requests.size===0&&confirmReleases.length===0&&!failConfirm);
    assert.equal(await page.locator('.daily-finish-backdrop').count(),0,'A failed automatic save cannot reopen its retry after navigation');
    ...
    await page.locator('[data-assignment-id="r2-a1"]').click();await page.waitForSelector('.daily-result-dialog');
    assert.equal(await page.evaluate(()=>results.r2.all_completed),true,'Another rider automatically saves their full list after a teammate’s partial finish');
    await page.locator('[data-keep-riding]').click();
    ...
    assert.equal(await page.locator('[data-finish-daily-athlete="r2"]').textContent(),'View Daily result','Light refresh preserves the other rider’s automatic full finish');
    assert.deepEqual(errors,[]);
    console.log('PASS actual rider/coach Daily controls, automatic full finish and queued saving, partial 0/N/some/N confirmation/cancel, unticked preservation, zero partial awards/no PB, authoritative reload with stopped timer, other categories and riders remain active, repeat safety, final-tap timing, same-candidate retries, navigation failure suppression, excluded sharing and mobile/tablet layout. No live requests.');
