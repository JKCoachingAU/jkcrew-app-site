...
async function run(){
  ...
  try{
    ...
    const stale=await create();let release;const gate=new Promise(resolve=>release=resolve);let seen;const requested=new Promise(resolve=>seen=resolve);await stale.route('**/bike-three-model.js*',async route=>{seen();await gate;await route.continue();});await stale.evaluate(()=>testMount('old-owner'));await requested;await stale.evaluate(()=>{JKCrewBikeGarage.destroy();document.querySelector('#view').innerHTML='<p>New account page</p>';});release();await stale.evaluate(()=>Promise.allSettled(testMountPromises));equal(await stale.locator('#view').innerHTML(),'<p>New account page</p>','A late module load cannot repaint a disposed private view');equal(await stale.locator('.bike-three-canvas').count(),0,'Stale loading does not attach a renderer');await stale.close();
