...
window.testMount=(owner='orbit-owner',open360=true)=>{testOwner=owner;JKCrewBikeGarage.mount({root:document.querySelector('#view'),client:testClient,userId:owner,isCurrent:()=>testOwner===owner,onBack:()=>{}});if(open360)document.querySelector('[data-bike-view-toggle]').click();};
...
async function run(){
  ...
  try{
    const page=await create();check(!local.requests.some(file=>file.startsWith('/vendor/')),'Loading the garage script alone does not download WebGL dependencies');await page.evaluate(()=>testMount('orbit-owner',false));
    await page.waitForFunction(()=>document.querySelector('[data-bike-art][data-view="photo"] .jkcrew-bike-art')&&document.querySelector('[data-bike-art]').getAttribute('aria-busy')==='false');
    check(!local.requests.some(file=>file.startsWith('/vendor/')),'The default photographic editor does not download WebGL dependencies');
    equal(await page.evaluate(()=>testMounts.length),0,'Photo view starts without a GPU renderer');
    await page.locator('[data-bike-name]').fill('My photographic build');
    await category(page,'Frame');await page.locator('[data-bike-colour="#F26879"]').click();await page.locator('[data-bike-sheet-close]').click();
    await page.screenshot({path:'/tmp/jkcrew-photo-default-edited.png'});
    await page.locator('[data-bike-view-toggle]').click();await ready(page);
    const originalDraft=await page.evaluate(()=>JSON.stringify(testDraft()));
    await page.locator('[data-bike-view-toggle]').click();
    await page.waitForFunction(()=>document.querySelector('[data-bike-art][data-view="photo"] .jkcrew-bike-art'));
    check(await page.evaluate(()=>testMounts.every(m=>m.disposed)),'Returning to the photograph releases the GPU renderer');
    equal(await page.evaluate(()=>JSON.stringify(testDraft())),originalDraft,'Switching views preserves the full bike draft');
    await page.locator('[data-bike-view-toggle]').click();await ready(page);
    equal(await page.locator('.bike-three-canvas').count(),1,'Reopening 360 mounts exactly one renderer');
    equal(await page.evaluate(()=>JSON.stringify(testDraft())),originalDraft,'The edited name and frame colour survive reopening 360');
    ...
    const stale=await create();let release;const gate=new Promise(resolve=>release=resolve);let seen;const requested=new Promise(resolve=>seen=resolve);await stale.route('**/bike-three-model.js*',async route=>{seen();await gate;await route.continue();});await stale.evaluate(()=>testMount('old-owner'));await requested;await stale.locator('[data-bike-view-toggle]').click();
    release();await stale.evaluate(()=>Promise.allSettled(testMountPromises));
    equal(await stale.locator('[data-bike-art]').getAttribute('data-view'),'photo','A cancelled 3D startup cannot replace Photo view');
    equal(await stale.locator('.bike-three-canvas').count(),0,'Switching back during startup leaves no hidden GPU canvas');
