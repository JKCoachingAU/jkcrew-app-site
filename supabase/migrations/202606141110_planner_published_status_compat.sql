-- Keep legacy "published" weekly plans active while the app moves to the
-- explicit scheduled_next_week status.
...
begin
  ...
  where coach_id = v_coach
    ...
    and status in ('draft', 'draft_next_week', 'scheduled_next_week', 'published');
...
begin
  ...
  where plan.athlete_id = p_athlete_id
    ...
    and plan.status in ('scheduled_next_week', 'published', 'draft_next_week', 'draft')
  order by case plan.status when 'scheduled_next_week' then 0 when 'published' then 1 when 'draft_next_week' then 2 else 3 end
  limit 1;
