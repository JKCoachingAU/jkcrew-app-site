-- Preserve the existing whole-team stake and payout rules while allowing
-- five points per rider for teams of five or six (25 or 30 per team).
-- Existing battles, results, API signatures and participant permissions are unchanged.
alter table public.weekly_rider_battles
  drop constraint weekly_rider_battles_reward_points_check;
alter table public.weekly_rider_battles
  add constraint weekly_rider_battles_reward_points_check
  check (reward_points between 1 and greatest(20, battle_size * 5));
